...
Requires:  pip install streamlit yfinance plotly scikit-learn pandas numpy
"""
...
def forecast_gb(model, last_row, scaler_X, n_days):
    ...
    for _ in range(n_days):
        ...
        preds.append(pred)
        row[close_idx] = pred  # feed prediction back as next Close
    return preds
...
with st.sidebar:
    ...
    lookback = st.slider("Lookback (days, for display)", 30, 120, 60, step=10)
...
st.caption("Live data · Gradient Boosting · Technical analysis · Buy/Sell signals")
...
with tab2:
    ...
    if not train_model:
        st.info("👈 Click **Train Models** in the sidebar to train Gradient Boosting, Random Forest & Linear Regression.")
    else:
        ...
        #  Gradient Boosting (replaces LSTM) 
        ...
        mets = {
            ...
            "Gradient Boosting":  mk_metrics(y_test, gb_preds),
        }
        ...
        fig_cmp.add_trace(go.Scatter(x=test_dates, y=gb_preds, name="Gradient Boosting",
        ...
        feat_df = pd.DataFrame({"Feature": FEATURE_COLS, "Importance": gb.feature_importances_})
        ...
        fig_imp.update_layout(title="Gradient Boosting Feature Importances (Top 15)",
...
with tab3:
    ...
    if "gb_model" not in st.session_state:
        ...
        gb_model_fc  = st.session_state["gb_model"]
        scaler_X_fc  = st.session_state["scaler_X"]
        last_row_fc  = st.session_state["last_row"]

        with st.spinner("Generating forecast…"):
            future_prices = forecast_gb(
                gb_model_fc, last_row_fc, scaler_X_fc, forecast_days
            )
